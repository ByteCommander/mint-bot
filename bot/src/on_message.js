          ...
          collector.on("collect", async (m) => {
            if (m.content.toUpperCase() === captcha.value) {
              if (guildChannels.autoMod == "strict") {
                ...
                const background = await Canvas.loadImage("../assets/mintwelcome.png");
    ...
    if (message.channel.id === "642882039372185609") {
      if (message.author.id !== "440892659264126997") {
        ...
        if (message.content.startsWith(prefix + "channel")) {
          let readname = fs
            .readFileSync("./storage/channelset.txt")
            .toString()
            .split("\n");
        ...
        if (message.content.startsWith(prefix + "set")) {
          fs.writeFile("./storage/channelset.txt", cargs, function (err) {
            if (err) throw err;
          });
          return message.channel.send(
